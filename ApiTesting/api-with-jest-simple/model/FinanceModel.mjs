...
class FinanceModel {
    ...
    static async getFinanceData({company='AAPL', region='US', interval='2m', range='1d'}={}) {

        // TODO: the value should contain template variables
        const params = `chart/${company}?${region}&lang=en-US&includePrePost=false&${interval}&${range}&corsDomain=finance.yahoo.com&.tsrc=finance`;
        try {
            return await axios.get(AppConfig.baseUrl + params);
        } catch (e) {
            return e.response;
